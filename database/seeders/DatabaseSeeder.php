...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if ($university) {
            UniversitySetting::updateOrCreate(
                ...
                [
                    ...
                    'admissions_email' => 'admissions@scale-state.scalecampuslab.test',
                    'admissions_phone' => '+1 555 0147',
                    'outreach_contact_name' => 'Noah Rivera',
                    'outreach_contact_email' => 'outreach@scale-state.scalecampuslab.test',
                    'outreach_contact_phone' => '+1 555 0145',
                    'emergency_contact_name' => 'Campus Safety Desk',
                    ...
                    'public_profile_summary' => 'A public research university offering practical, career-connected programmes for school groups and prospective students.',
                    'about' => 'Scale State University combines research-led teaching with applied learning across engineering, business, health sciences, arts, education, and public service. The university hosts structured school programmes that connect students with faculty, current undergraduates, admissions teams, and campus support services.',
                    'academic_strengths' => 'Engineering and technology, data science, business analytics, health sciences, teacher education, digital media, environmental studies, entrepreneurship, and interdisciplinary honors pathways.',
                    'student_support_services' => 'Academic advising, disability services, tutoring, career coaching, scholarship counseling, wellbeing support, student success coaching, and first-generation student programmes.',
                    'international_student_support' => 'Dedicated international admissions advising, visa guidance, orientation, English language support, cultural transition programming, and peer mentoring.',
                    'accessibility_services' => 'Accessible routes are available for all standard visit locations. Schools can request mobility support, sign language interpretation, quiet spaces, and accessible seating during registration.',
                    'visit_policy' => 'School visits require an approved request, final attendee numbers before arrival, one adult chaperone for every 15 students, and compliance with university campus safety guidance.',
                    'safety_policy' => 'All visiting groups must check in at the designated welcome point, wear visitor badges where provided, and follow instructions from campus staff during tours, labs, and demonstrations.',
                    'campus_map_url' => 'https://scale-state.scalecampuslab.test/map',
                    'virtual_tour_url' => 'https://youtube.com/watch?v=dQw4w9WgXcQ',
                    'facebook_url' => 'https://facebook.com/scalestate',
                    'linkedin_url' => 'https://linkedin.com/school/scale-state-university',
                    'instagram_url' => 'https://instagram.com/scalestate',
                    'youtube_url' => 'https://youtube.com/@scalestate',
                    'logo_url' => null,
                    'brand_color' => '#006a61',
        ...
        $schools = [
            ['Lincoln High School', 'LHS-PARTNER', 'Cityville', 'ST', 'Midwest US', 'United States', 'public', 'high', 1320, 3.4, 89, 34, 'Cityville Unified District', 'Dr. Evelyn Carter', 'Jane Doe', 'AP, Honors, STEM pathway, dual-credit college courses', 'Semester', 1240, 'Grades 9-12'],
            ['Oakwood Preparatory Academy', 'OPA-DEMO', 'Greenwich', 'CT', 'Northeast US', 'United States', 'private', 'elite', 1480, 4.2, 98, 12, 'Fairfield County Independent Schools', 'Dr. Margaret Ellison', 'Claire Whitman', 'IB Diploma, AP Capstone, entrepreneurship, research seminar', 'Trimester', 860, 'Grades 9-12'],
            ['North Valley Science Magnet', 'NVSM-DEMO', 'San Jose', 'CA', 'West Coast', 'United States', 'public', 'high', 1420, 2.8, 91, 8, 'Santa Clara STEM Consortium', 'Dr. Luis Moreno', 'Priya Nair', 'STEM magnet, AP sciences, robotics, data science pathway', 'Semester', 1120, 'Grades 9-12'],
            ['International School of Boston', 'ISB-DEMO', 'Boston', 'MA', 'Northeast US', 'United States', 'ib_school', 'high', 1510, 5.1, 95, 18, 'Greater Boston International Network', 'Helene Marchand', 'Sophie Laurent', 'IB Diploma, bilingual French-English curriculum, global studies', 'Semester', 720, 'Grades 6-12'],
            ['Westlake Arts and Tech Academy', 'WATA-DEMO', 'Austin', 'TX', 'South Central', 'United States', 'charter', 'emerging', 1390, 2.1, 84, 3, 'Austin Innovation Charter Network', 'Marcus Lee', 'Tanya Brooks', 'Project-based arts, media technology, UX, computer science', 'Quarter', 540, 'Grades 9-12'],
            ['NIST International School', 'NIST-DEMO', 'Bangkok', 'Bangkok', 'Southeast Asia', 'Thailand', 'ib_school', 'elite', 1500, 6.4, 97, 24, 'Bangkok International Schools Association', 'Anong Srisai', 'Michael Tan', 'IB Primary Years, Middle Years, Diploma, global leadership', 'Semester', 1680, 'Early Years-Grade 12'],
            ['St. Georges International School', 'SGIS-DEMO', 'Geneva', 'Geneva', 'Europe', 'Switzerland', 'private', 'stable', 1460, 3.6, 88, 9, 'Lake Geneva International Education Group', 'Dr. Elise Baumann', 'Marc Dubois', 'IB Diploma, Swiss maturity preparation, multilingual studies', 'Trimester', 940, 'Grades 6-12'],
        ];
        ...
        foreach ($schools as [$name, $code, $city, $state, $region, $country, $type, $tier, $sat, $yield, $score, $applicants, $district, $principal, $counselor, $curriculum, $calendar, $studentCount, $gradeRange]) {
            ...
            $schoolProfile = School::updateOrCreate(
                ...
                [
                    'name' => $name,
                    'school_type' => $type,
                    'institution_level' => str_contains($gradeRange, 'Early') || str_contains($gradeRange, '6-12') ? 'middle_high_school' : 'high_school',
                    'ownership' => $type === 'public' ? $district : Str::title(str_replace('_', ' ', $type)).' governing board',
                    'location' => "{$city}, {$region}, {$country}",
                    'website' => "https://{$slug}.scalecampuslab.test",
                    'main_phone' => '+1 555 '.str_pad((string) rand(200, 899), 3, '0', STR_PAD_LEFT).' '.str_pad((string) rand(1000, 9999), 4, '0', STR_PAD_LEFT),
                    'address' => rand(100, 999).' College Road',
                    'city' => $city,
                    'state' => $state,
                    'country' => $country,
                    'district' => $district,
                    'region' => $region,
                    'timezone' => $country === 'Thailand' ? 'Asia/Bangkok' : ($country === 'Switzerland' ? 'Europe/Zurich' : 'America/New_York'),
                    'coordinator_name' => $counselor,
                    'coordinator_email' => "counselor@{$slug}.scalecampuslab.test",
                    'coordinator_phone' => '+1 555 '.str_pad((string) rand(200, 899), 3, '0', STR_PAD_LEFT).' '.str_pad((string) rand(1000, 9999), 4, '0', STR_PAD_LEFT),
                    'principal_name' => $principal,
                    'principal_email' => "principal@{$slug}.scalecampuslab.test",
                    'counselor_name' => $counselor,
                    'counselor_email' => "counselor@{$slug}.scalecampuslab.test",
                    'counselor_phone' => '+1 555 '.str_pad((string) rand(200, 899), 3, '0', STR_PAD_LEFT).' '.str_pad((string) rand(1000, 9999), 4, '0', STR_PAD_LEFT),
                    'admissions_email' => "admissions@{$slug}.scalecampuslab.test",
                    'registrar_email' => "registrar@{$slug}.scalecampuslab.test",
                    'emergency_contact_name' => 'Visit Safety Desk',
                    'emergency_contact_phone' => '+1 555 911 '.str_pad((string) rand(1000, 9999), 4, '0', STR_PAD_LEFT),
                    'emergency_contact_email' => "safety@{$slug}.scalecampuslab.test",
                    'grade_range' => $gradeRange,
                    'student_count' => $studentCount,
                    'accreditation' => 'Regionally accredited with annual college-readiness review',
                    'curriculum' => $curriculum,
                    'academic_calendar' => $calendar,
                    'graduation_rate' => min(99, 88 + ($score / 10)),
                    'average_class_size' => max(16, min(30, (int) round($studentCount / 45))),
                    'boarding_available' => in_array($country, ['Thailand', 'Switzerland'], true),
                    'international_students' => in_array($type, ['ib_school', 'private'], true),
                    'student_support_services' => 'College counseling, accessibility support, academic advising, mental health referral, scholarship guidance, and supervised visit preparation.',
                    'transportation_notes' => 'School coordinates approved group transportation, chaperone assignments, final rosters, dietary needs, and arrival timing before each university visit.',
                    'visit_policy' => 'Final student roster, emergency contacts, school approval, and chaperone coverage must be confirmed before travel. Student changes are reviewed by the school coordinator.',
                    'safety_policy_url' => "https://{$slug}.scalecampuslab.test/safety",
                    'facebook_url' => "https://facebook.com/{$slug}",
                    'linkedin_url' => "https://linkedin.com/school/{$slug}",
