...
class DashboardController extends Controller
{
    ...
    public function updateSchoolSettings(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'school_code' => ['nullable', 'string', 'max:40', Rule::unique('schools', 'school_code')->ignore($request->user()->school_id)],
            ...
            'safety_policy_url' => ['nullable', 'url', 'max:500'],
    ...
    public function updateUniversitySettings(Request $request): RedirectResponse
    {
        ...
        $existingSetting = UniversitySetting::query()
            ->where('university_user_id', $request->user()->id)
            ->first();

        $validated = $request->validate([
            'institution_name' => ['required', 'string', 'max:180'],
            'institution_code' => ['nullable', 'string', 'max:40', Rule::unique('university_settings', 'institution_code')->ignore($existingSetting?->id)],
            'institution_type' => ['nullable', 'string', 'max:100'],
            'ownership' => ['nullable', 'string', 'max:80'],
            'accreditation' => ['nullable', 'string', 'max:255'],
            'founded_year' => ['nullable', 'string', 'max:12'],
            'website' => ['nullable', 'url', 'max:255'],
            ...
            'admissions_email' => ['nullable', 'email:rfc', 'max:180'],
            'admissions_phone' => ['nullable', 'string', 'max:80'],
            'outreach_contact_name' => ['nullable', 'string', 'max:160'],
            'outreach_contact_email' => ['nullable', 'email:rfc', 'max:180'],
            'outreach_contact_phone' => ['nullable', 'string', 'max:80'],
            'emergency_contact_name' => ['nullable', 'string', 'max:160'],
            'emergency_contact_phone' => ['nullable', 'string', 'max:80'],
            'address' => ['nullable', 'string', 'max:255'],
            'city' => ['nullable', 'string', 'max:120'],
            ...
            'public_profile_summary' => ['nullable', 'string', 'max:500'],
            'about' => ['nullable', 'string', 'max:4000'],
            'academic_strengths' => ['nullable', 'string', 'max:3000'],
            'student_support_services' => ['nullable', 'string', 'max:3000'],
            'international_student_support' => ['nullable', 'string', 'max:3000'],
            'accessibility_services' => ['nullable', 'string', 'max:3000'],
            'visit_policy' => ['nullable', 'string', 'max:3000'],
            'safety_policy' => ['nullable', 'string', 'max:3000'],
            'campus_map_url' => ['nullable', 'url', 'max:500'],
        ...
        UniversitySetting::updateOrCreate(
            ...
            [
                ...
                'admissions_email' => $validated['admissions_email'] ?? null,
                'admissions_phone' => $validated['admissions_phone'] ?? null,
                'outreach_contact_name' => $validated['outreach_contact_name'] ?? null,
                'outreach_contact_email' => $validated['outreach_contact_email'] ?? null,
                'outreach_contact_phone' => $validated['outreach_contact_phone'] ?? null,
                'emergency_contact_name' => $validated['emergency_contact_name'] ?? null,
                'emergency_contact_phone' => $validated['emergency_contact_phone'] ?? null,
                ...
                'public_profile_summary' => $validated['public_profile_summary'] ?? null,
                'about' => $validated['about'] ?? null,
                'academic_strengths' => $validated['academic_strengths'] ?? null,
                'student_support_services' => $validated['student_support_services'] ?? null,
                'international_student_support' => $validated['international_student_support'] ?? null,
                'accessibility_services' => $validated['accessibility_services'] ?? null,
                'visit_policy' => $validated['visit_policy'] ?? null,
                'safety_policy' => $validated['safety_policy'] ?? null,
                'campus_map_url' => $validated['campus_map_url'] ?? null,
    ...
    private function universitySettings(User $user): array
    {
        $settings = UniversitySetting::query()->firstOrCreate(
            ...
            [
                ...
                'admissions_email' => $user->email,
                'outreach_contact_name' => $user->name,
                'outreach_contact_email' => $user->email,
                'public_profile_summary' => 'University profile details can be completed from the Profile tab.',
                'brand_color' => '#006a61',
                'default_visit_config' => [
                    'capacity' => 80,
        ...
        return [
            'profile' => [
                ...
                'admissionsEmail' => $settings->admissions_email,
                'admissionsPhone' => $settings->admissions_phone,
                'outreachContactName' => $settings->outreach_contact_name,
                'outreachContactEmail' => $settings->outreach_contact_email,
                'outreachContactPhone' => $settings->outreach_contact_phone,
                ...
                'publicProfileSummary' => $settings->public_profile_summary,
                'about' => $settings->about,
                'academicStrengths' => $settings->academic_strengths,
                'studentSupportServices' => $settings->student_support_services,
                'internationalStudentSupport' => $settings->international_student_support,
                'accessibilityServices' => $settings->accessibility_services,
                'visitPolicy' => $settings->visit_policy,
                'safetyPolicy' => $settings->safety_policy,
                'campusMapUrl' => $settings->campus_map_url,
                'virtualTourUrl' => $settings->virtual_tour_url,
            ...
            'programContactDefaults' => [
                'contactName' => $settings->outreach_contact_name ?: ($settings->primary_contact_name ?: $user->name),
                'contactTitle' => 'Admissions / outreach office',
                'contactEmail' => $settings->outreach_contact_email ?: ($settings->admissions_email ?: ($settings->primary_contact_email ?: $user->email)),
                'contactPhone' => $settings->outreach_contact_phone ?: ($settings->admissions_phone ?: ($settings->primary_contact_phone ?: $user->phone)),
                'contactOffice' => $settings->institution_name ?: $user->name,
                'contactWebsite' => $settings->website,
            ],
    ...
    private function events(?int $universityUserId = null, ?array $statuses = null, ?array $eventIds = null): array
    {
        return CampusEvent::query()
            ...
            ->map(fn (CampusEvent $event) => [
                ...
                'shareUrl' => $event->share_slug ? route('programs.public.show', $event->share_slug) : null,
        ...
        return $targetSchools
            ->map(function (TargetSchool $school) use ($linkedSchoolsByName, $linkedSchoolsByCode): array {
                ...
                return [
                    ...
                    'name' => $school->name,
                    'logoUrl' => $linkedSchool?->logo_url,
                    'website' => $linkedSchool?->website,
                    'mainPhone' => $linkedSchool?->main_phone,
                    'address' => $linkedSchool?->address,
                    'city' => $school->city,
                    ...
                    'coordinatorPhone' => $linkedSchool?->coordinator_phone,
                    'principalName' => $linkedSchool?->principal_name,
                    'principalEmail' => $linkedSchool?->principal_email,
                    'counselorName' => $linkedSchool?->counselor_name,
                    'counselorEmail' => $linkedSchool?->counselor_email,
                    'counselorPhone' => $linkedSchool?->counselor_phone,
                    'emergencyContactName' => $linkedSchool?->emergency_contact_name,
                    'emergencyContactPhone' => $linkedSchool?->emergency_contact_phone,
                    'emergencyContactEmail' => $linkedSchool?->emergency_contact_email,
                    'status' => $school->status ?: 'verified',
                    'type' => $school->school_type,
                    'institutionLevel' => $linkedSchool?->institution_level,
                    'ownership' => $linkedSchool?->ownership,
                    ...
                    'gradeRange' => $linkedSchool?->grade_range,
                    'studentCount' => $linkedSchool?->student_count,
                    'accreditation' => $linkedSchool?->accreditation,
                    'curriculum' => $linkedSchool?->curriculum,
                    'academicCalendar' => $linkedSchool?->academic_calendar,
                    'graduationRate' => $linkedSchool?->graduation_rate,
                    'averageClassSize' => $linkedSchool?->average_class_size,
                    'boardingAvailable' => $linkedSchool ? (bool) $linkedSchool->boarding_available : null,
                    'internationalStudents' => $linkedSchool ? (bool) $linkedSchool->international_students : null,
                    'studentSupportServices' => $linkedSchool?->student_support_services,
                    'transportationNotes' => $linkedSchool?->transportation_notes,
                    'visitPolicy' => $linkedSchool?->visit_policy,
                    'safetyPolicyUrl' => $linkedSchool?->safety_policy_url,
    ...
    private function schoolProfile(User $user): array
    {
        ...
        return [
            ...
            'institutionLevel' => $school->institution_level,
            'ownership' => $school->ownership,
            'location' => $school->location,
            'logoUrl' => $school->logo_url,
            'website' => $school->website,
            'mainPhone' => $school->main_phone,
            ...
            'registrarEmail' => $school->registrar_email,
            'coordinatorPhone' => $school->coordinator_phone,
            'principalName' => $school->principal_name,
            'principalEmail' => $school->principal_email,
            'counselorName' => $school->counselor_name,
            'counselorEmail' => $school->counselor_email,
            'counselorPhone' => $school->counselor_phone,
            'emergencyContactName' => $school->emergency_contact_name,
            'emergencyContactPhone' => $school->emergency_contact_phone,
            'emergencyContactEmail' => $school->emergency_contact_email,
            'gradeRange' => $school->grade_range,
            'studentCount' => $school->student_count,
            'accreditation' => $school->accreditation,
            'curriculum' => $school->curriculum,
            'academicCalendar' => $school->academic_calendar,
            'graduationRate' => $school->graduation_rate,
            'averageClassSize' => $school->average_class_size,
            'boardingAvailable' => (bool) $school->boarding_available,
            'internationalStudents' => (bool) $school->international_students,
            'studentSupportServices' => $school->student_support_services,
            'transportationNotes' => $school->transportation_notes,
            'visitPolicy' => $school->visit_policy,
            'safetyPolicyUrl' => $school->safety_policy_url,
            'facebookUrl' => $school->facebook_url,
            'linkedinUrl' => $school->linkedin_url,
